...
def prodExp(prod, name, extra=[]):
    runExp(
        prod,
        simpJava,
        tableFile="Tables/" + name + "Java.table",
        Nlist=llNlist,
        extra=extra,
    )
    runExp(
        prod,
        fastHash,
        tableFile="Tables/" + name + "JavaHash.table",
        Nlist=llNlist,
        extra=extra,
    )
    runExp(
        prod,
        dictPyth,
        tableFile="Tables/" + name + "PythDict.table",
        Nlist=llNlist,
        extra=extra,
    )
    runExp(
        prod,
        simpPyth,
        tableFile="Tables/" + name + "PythSimp.table",
        Nlist=Nlist,
        extra=extra,
    )
    runExp(
        prod,
        fastPyth,
        tableFile="Tables/" + name + "PythFast.table",
        Nlist=lNlist,
        extra=extra,
    )
    runExp(
        prod,
        fastCmp,
        tableFile="Tables/" + name + "FastCmp.table",
        Nlist=llNlist,
        extra=extra,
    )
...
def runExp(producer, foursum, tableFile="", Nlist=[100], extra=[]):
    ...
    for i in range(4):
        for N in Nlist:
            ...
            ps = subprocess.Popen(
                tuple(list(producer) + [str(N)] + extra),
                stdout=open("tmp.in", "w"),
            ...
            output = subprocess.check_output(
                foursum, stdin=open("tmp.in", "r"), stderr=subprocess.DEVNULL
            )
